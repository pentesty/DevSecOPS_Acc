...
//     }
      ...
      stage ('SAST-SemGrep') {
	      steps {
	        ...
	        sh '''
		ssh -o  StrictHostKeyChecking=no ubuntu@172.31.18.71 'sudo git clone https://github.com/pentesty/DevSecOps_Acc.git && sudo cd DevSecOps_Acc && sudo docker run --rm -v "${PWD}:/src" returntocorp/semgrep semgrep --config auto  --output scan_results.json --json'
		'''
		     }
