...
class Log:
    ...
    def logfile(self, filename: str, log_level: int = logging.INFO) -> None:
        ...
        for hdlr in self.root_logger.handlers:
            # Setting this to isinstance(hdlr, logging.StreamHandler) seems to be set to True, even if
            # its a logging filehandler
            # Example:
            # >>> type(fh) == logging.StreamHandler
            # False
            # >>> isinstance(fh, logging.StreamHandler)
            # True
